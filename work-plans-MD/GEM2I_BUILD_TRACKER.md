...
> **CLAIMED BY:** (unclaimed — session 6 FINISHED 2026-09-23, Carlos M. Artiles: port from Carlos 1.A→1.H + Carlos levels/enrollment + My Community perf + Phase 6 automatable items + Phase 7 runbook, all DEPLOYED/pushed. Next session starts from the human list: walk-through, e2e with accounts, content (Terms!), Stripe live, cutover GO per GEM2I_CUTOVER_RUNBOOK.md; B3/B5 award rules when the owner defines them)
> **PHASE 6/7 (session 6, later, 2026-09-23):** owner ruling — points stay exactly as migrated; award rules (B3/B5) defined later by the owner (NOT blocking). ✅ Reward-points sub-editor in the Events manager (legacy fields editable as-is, 9760e33) · ✅ B1 external-ticket passthrough confirmed (51 events, link on the detail page) · ✅ **Phase 7 prepared: `work-plans-MD/GEM2I_CUTOVER_RUNBOOK.md`** (preconditions, delta ETL order, domain switch incl. `REACT_APP_BACKEND_URL`/CORS/nginx/cert/Site URL/Stripe webhook, verification, documented rollback) · ✅ **delta-safe loaders**: catalogs/history skip docs edited in the CMS (`updated_at`), members keep profiles edited in the new system and new members land on Nivel 0 (`--overwrite-edited` to force). Remaining = human: walk-through, e2e with accounts, content pages (Terms still shows fork text "Legacy Consulting"), Stripe live, DNS/cutover GO.
> **PHASE 6 (session 6 cont., 2026-09-23):** ✅ i18n sweep of the public theme (last a11y strings EN/ES) · ✅ design audit: impeccable detector 0 findings on all gem2i pages/components, no horizontal overflow at 375px (home/events/artists/venues/artist detail) · ✅ My Community tree 1 query ($graphLookup + `sponsor_id_1` index, GEM-3 1228→230 ms). ⏸ **B3 points writes + B5 share→referral BLOCKED on the legacy rules**: events carry `points.legacy_eticket` {purchase_for_self 53 events, each_additional 35, purchase_for_a_friend 36, invite_to_purchase_share 51, friend_purchased_via_invite 51} and `points.legacy_guest_list` {yourself 9, add_friend 8, invite_friends 8, list_registration 7, list_check_in 7}; exact award semantics live in the legacy PHP (`points-invite-event.php`, `event_payment.php`, `ajax-event-add-gues-list.php`, `event-info.php`), not on this machine and the legacy box (18.208.85.155) refuses SSH from here → owner to provide those files. Still open for Phase 6: operator e2e "new e-ticket event from the CMS alone" (needs admin login — human).
> **PORT FROM CARLOS ✅ (session 6, 2026-09-23):** 70 modules → 19 done (all live on beta) · 31 waiting on a human (H1–H17 in the plan) · 14 skipped · 6 identical. Real gem2i bugs fixed on the way: member logins weren't recorded, membership numbers could duplicate, `/membership-enrollment` had no backend, sponsors showed `AUX-n`. New: admin-by-role, Invite Code/QR, My Community (privacy b), field visibility, Waiting List modal + emails, image cropper. Next: CMS config H1/H4/H6/H11 + e2e H3; decisions H7–H17.
> **P4/5 HISTORY ETL ✅ (session 5 cont., 2026-07-17, data-only):** 1704 events got legacy tiers (93 w/ real prices from the dated ladders, kept in legacy_prices)/guest_list benefits (109, det member_types ids)/points/payment.currency/fees · 240 legacy transactions archived (legacy:true; economics verbatim) · 6457 points-history rows + 16-action catalog. NEW scripts gem2i_etl_history.py / gem2i_load_history.py. No deploy needed.
...
> **LAST SYNC:** 2026-09-23 (session 6, Carlos's machine — see the session-6 entry in memory/PROGRESS.md; box state: 1,736 members on Nivel 0 with level enforcement ON, enrollment = Carlos' 50 fields, `sponsor_id_1` index, backups in /opt/_port_backups/).

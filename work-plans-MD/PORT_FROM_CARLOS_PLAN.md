...
STATUS     : ✅ PLAN RECORRIDO COMPLETO 1.A → 1.H (2026-09-23). 70 módulos: 20 hechos · 30 esperando
             humano · 14 saltados · 6 idénticos (#29 gobernanza y H10 enrollment resueltos
             por el dueño el mismo día). Todo lo hecho está DESPLEGADO en beta.gem2i.com y
...
| 29 | Gobernanza NIVEL/TIPO/ROL | **Hecho** (2026-09-23, decisión del dueño) | Niveles de Carlos migrados y aplicados en servidor (`enforce`); tipos legacy de Gem2i intactos. Ver H8 |
...
| ~~H8~~ | #29 Gobernanza | ✅ **RESUELTO 2026-09-23 (Carlos M. Artiles): "migra los niveles de Carlos; la etiqueta se puede renombrar, la funcionalidad de acceso por niveles no".** Código portado y desplegado (commit 6ef4b21: `level_access`/`product_access`/`member_capabilities`/`site_pages`, gate en `get_current_member`, editor de niveles, defaults al alta) + datos migrados con `scripts/gem2i_gov_migrate_levels.js` (adaptación de `gov_migrate_brand.js` de Carlos): **Nivel 0 / Nivel 1** con los permisos de Carlos, `enforce` + `membership_v2_enabled`, 1.736 miembros → Nivel 0 (admin intacto). **Diferencias forzadas por los datos de Gem2i:** sin quick links (Gem2i no tiene Home/KMS/LMS) y **tipos de miembro intactos** (los 5 legacy mantienen los beneficios de guest list; 83 miembros tipados). Respaldos en `/opt/_port_backups/gov_*_before.json`. Verificado con el código desplegado: Nivel 0 → Perfil/Sponsor/QR/guest list/tickets permitidos; **Invite Code y My Community denegados (igual que en Carlos)**; admin todo permitido | Si los miembros deben usar Invite Code / My Community: CMS → Member Levels → marcar esas secciones en el nivel (config, sin código). Si los niveles vivos de Carlos cambiaron desde su script (editados en su CMS), pasar un export de `member_levels` de su backup y se cargan igual |
| (histórico) | #29 Gobernanza NIVEL/TIPO/ROL | Decidir el modelo de acceso de Gem2i: hoy los 1.737 miembros **no tienen nivel** (ven todo) y existen "Level 1–4" heredados del fork. La gobernanza de Carlos cambia qué ve cada miembro (niveles aplicados en servidor, capacidades por tipo, páginas por nivel, nivel/tipo por defecto al registrarse) | Primero definir los niveles de negocio de Gem2i (¿qué ve un miembro normal vs promotor vs VIP?) con Anthony; después portar. Sin esa definición portarlo solo añade código inerte o cambia accesos sin querer |
...
| ~~H18~~ | #22 My Community (rendimiento) | ✅ **Índice `sponsor_id_1` creado + endpoint reescrito a una sola consulta `$graphLookup`** (commit 4b3d7cc). Verificado en la caja contra el algoritmo viejo: árboles idénticos para GEM-3/61/1347; GEM-3 **1.228 ms → 230 ms** medido desde el backend | **Aplica también a Carlos Beta (ver nota abajo)** |

> **Nota para Carlos Beta (AUX-1.0):** `routes/membership.py::get_my_community` en Carlos @8a0d4dd tiene el mismo patrón de 1 consulta por nodo (lo usan My Account 1.0 y 2.0) y su código no crea índice en `members.sponsor_id`. Mejora recomendada, en una sesión del repo de Carlos: (1) `db.members.createIndex({sponsor_id:1})` en su BD; (2) portar el cuerpo nuevo de `get_my_community` de Gem2i (commit 4b3d7cc) añadiendo el campo `account_status` que su versión devuelve por nodo. Cuánto gana depende del tamaño de sus redes (no medible desde aquí: su caja está fuera de alcance).
| (histórico) | #22 My Community (rendimiento) | **Hallado en el e2e:** el árbol del mayor sponsor legacy (GEM-3: 1.625 personas en 8 niveles) tarda **7,2 s** en construirse (1 consulta por nodo, sin índice en `members.sponsor_id`) | Crear índice (no único, reversible): `db.members.createIndex({sponsor_id:1},{name:"sponsor_id_1"})` — cambio en BD, por eso queda aquí |
| ~~H19~~ | #11 Operator Email | ✅ Confirmado en BD 2026-09-23: `operator_email` guardado | — |
| (histórico) | #11 Operator Email | En BD `settings.operator_email` sigue **vacío** (consulta 2026-09-23 tras "hecho"). Sin él no llega el aviso al operador de cada alta en Waiting List (el email al suscriptor sí sale) | Revisar CMS → Settings → Email → Operator Notifications y pulsar **Save** |

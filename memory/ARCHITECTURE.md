...
**Ported from Carlos (2026-09-23, PORT_FROM_CARLOS_PLAN):** admin = `models/database.py::is_admin()` (role "admin" OR cms role `role_admin`; frontend `lib/isAdmin.js`) — use it, never raw `role == "admin"`. New members go through `routes/membership.py::insert_member_with_retry` (atomic `counters{_id:"membership_number"}`); logins via `record_login_event` (called from `/auth/login` for non-CMS logins + registration). `routes/enrollment.py` (public `/membership-enrollment` 4-step form; collections `enrollment_fields/_content/_applications`). `utils/membership_prefix.py` realigns IDs when `settings.aux_prefix` changes. Frontend: `components/gem2i/Gem2iWaitingListModal.js` (mounted in Gem2iHeader), `lib/ctaActions.js` (hero CTA actions), `components/ImageAdjust.js` (opt-in `adjust` on ImageUpload), `lib/defaultImages.js`, My Account pages InviteCode/MyCommunity; `membership_settings.hidden_fields` → `ctx.hiddenFields` from MyAccountLayout.
**Built (P4/5 data):** legacy commerce history loaded — `gem_events.{tiers,guest_list,points,payment,legacy_prices,transaction_fee}` sub-docs (ETL: `scripts/gem2i_etl_history.py` + `gem2i_load_history.py`); `gem_transactions` archive rows tagged `legacy:true` (never written by the live path); `gem_points_history` + `gem_config {key:'points_actions'}`.

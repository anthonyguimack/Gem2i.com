...
function HeroSlidePreview({ form }) {
  ...
  // Resolve localized fields to a display string (first non-empty locale) for the preview.
  const t = adminText(form.title), st = adminText(form.subtitle), d = adminText(form.description);
  const b1 = adminText(form.button_text), b2 = adminText(form.button_2_text), b3 = adminText(form.button_3_text);
  const hasContent = t || st || d || b1 || b2 || b3 ||
      ...
      <div className="relative w-full h-full">
        {t && (
          <div className="absolute max-w-[55%] px-1" style={{ left: toL(form.title_x), top: toT(form.title_y, 50) }}>
            <div className="text-lg md:text-xl xl:text-2xl font-bold leading-tight [&_em]:italic [&_p]:m-0" style={{ fontFamily: headingFont, color: 'white' }} dangerouslySetInnerHTML={{ __html: t }} />
          </div>
        )}
        {st && (
          <div className="absolute max-w-[55%] px-1" style={{ left: toL(form.subtitle_x), top: toT(form.subtitle_y, 80) }}>
            <div className={`text-xs md:text-sm font-semibold [&_em]:italic [&_p]:m-0`} style={{ fontFamily: headingFont, color: isPB ? 'var(--color-primary, #c08552)' : 'white' }} dangerouslySetInnerHTML={{ __html: st }} />
          </div>
        )}
        {d && (
          <div className="absolute max-w-[45%] px-1" style={{ left: toL(form.description_x), top: toT(form.description_y, 120) }}>
            <div className="text-[10px] md:text-xs leading-relaxed [&_p]:m-0" style={{ color: 'rgba(255,255,255,0.7)', fontFamily: headingFont }} dangerouslySetInnerHTML={{ __html: d }} />
          </div>
        )}
        {(b1 || b2 || b3) && (
...
export default function HeroSlideForm() {
  ...
  const handleSave = async () => {
    if (!adminText(form.title).trim() && !adminText(form.subtitle).trim()) { toast.error('Title or Subtitle is required'); return; }
    setSaving(true);
    try {
      ...
      <div className={sectionCls}>
        ...
        <div className="space-y-4">
          <div>
            ...
            <LocalizedField value={form.title} onChange={v => setForm(p => ({...p, title: v}))} render={({ value, onChange }) => (
              <div className={quillCls}><RichTextEditor value={value} onChange={onChange} placeholder="Slide title..." /></div>
            )} />
          </div>
          <div>
            <Label className="text-xs text-slate-500 mb-2 block">Subtitle</Label>
            <LocalizedField value={form.subtitle} onChange={v => setForm(p => ({...p, subtitle: v}))} render={({ value, onChange }) => (
              <div className={quillCls}><RichTextEditor value={value} onChange={onChange} placeholder="Slide subtitle..." /></div>
            )} />
          </div>
          <div>
            <Label className="text-xs text-slate-500 mb-2 block">Description</Label>
            <LocalizedField value={form.description} onChange={v => setForm(p => ({...p, description: v}))} render={({ value, onChange }) => (
              <div className={quillCls}><RichTextEditor value={value} onChange={onChange} placeholder="Slide description..." /></div>
            )} />
          </div>
        </div>
      </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <LocalizedField value={form[textKey]} onChange={v => setForm(p => ({...p, [textKey]: v}))} render={({ value, onChange }) => (
                    <Input value={value || ''} onChange={e => onChange(e.target.value)} className="mt-1" placeholder={n === 1 ? 'e.g. Learn More' : 'Optional'} data-testid={`slide-btn${suf}-text`} />
                  )} />
                </div>
                <div>
